...
shared `~/.cargo/advisory-db`. That cache is mutable state shared with every
...
or a tool might mistake them for genuine. They exist so a test can assert that
the local database was the one actually read. `Cargo.toml` excludes this
directory from the published package so the fabrication never ships.
